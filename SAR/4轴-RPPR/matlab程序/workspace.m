...
q1_lim=[-150,150];%SAR系列，q1为角位移，单位°
q2_lim=[-425,5];%SAR系列，q2为线位移，单位mm
q3_lim=[-105,105];%SAR系列，q3为线位移，单位mm
q4_lim=[-180,180];%SAR系列，q4为角位移，单位°

%工作空间比较
workspace1(n,q1_lim,q2_lim,q3_lim,q4_lim);%无动态限位
% workspace2(n,q2_lim,q3_lim);%有动态限位
...
%关节矩阵赋值
% q1=zeros(n,1);
q1=q1_lim(1)+diff(q1_lim)*rand(n,1);
q2=q2_lim(1)+diff(q2_lim)*rand(n,1);%在正负限位范围内进行蒙特卡洛随机采样
q3=q3_lim(1)+diff(q3_lim)*rand(n,1);%在正负限位范围内进行蒙特卡洛随机采样
q4=q4_lim(1)+diff(q4_lim)*rand(n,1);
% q4=zeros(n,1);
q=[q1,q2,q3,q4];
...
function workspace2(n,q2_lim,q3_lim)
...
q1=zeros(n,1);
for i=1:n
    q2(i,1)=q2_lim(1)+diff(q2_lim)*rand;%在正负限位范围内进行蒙特卡洛随机采样
    ...
    q3(i,1)=q3_lim_new(1)+diff(q3_lim_new)*rand;%在新正负限位范围内进行蒙特卡洛随机采样
end
q4=zeros(n,1);
q=[q1,q2,q3,q4];
...
%笛卡尔空间位姿计算
x=(lambda*q(3)+a2+a3-a1)*cosd(q(1));
y=(lambda*q(3)+a2+a3-a1)*sind(q(1));
z=(lambda-1)*q(2)+d2+d3-d4;
a=q(1)-q(4);
b=0;
c=0;
