...
% 逆解计算
% q1
q1=atan2(py-ay*d6, px-ax*d6)-atan2(d4,flag(1)*sqrt((py-ay*d6)^2+(px-ax*d6)^2-d4^2));
if q1>=2*pi
    q1=q1-2*pi;
elseif q1<=-2*pi
    q1=q1+2*pi;
...
tmp=(tmp1^2+tmp2^2+a2^2-a3^2)/(2*a2);
q2=tmp1^2+tmp2^2-tmp^2;
if q2<0
    q=[0,0,0,0,0,0];
    return;
end
...
% q3
tmp1=tmp1+a2*sin(q2);%c23
tmp2=tmp2-a2*cos(q2);%s23
