...
qlast(1,:)=[215.175, 280.444, 97.444, 11.948, 29.885, 0];%初始角位移
p=fkine(qlast(1,:));%初始姿态

px=[214.2806, 296.5878, 82.6013, 0, 18.7360, -141.5970];%最终目标姿态

j=1;
for i=1:1:100;
    p1(j,:)=p+ (px-p)*0.01*i;%期望位姿
    q(j,:)=ikine(p1(j,:));%期望位姿的对应逆解,关节位移
    qlast(j+1,:)=q(j,:);%更新上一循环角位移
